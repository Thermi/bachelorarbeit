...
Der Code für  die Implementierung von handle\_plain() auf Windows ist auf~\pageref{lst:handle-plain-windows}
zu sehen. Der Code für die Implementierung auf Windows ist 239 Zeilen lang, was
deutlich mehr ist als bei anderen Funktionen. Das liegt daran, dass die Initialisierung
der Strukturen relativ aufwendig ist und mehrfach durchgeführt werden muss.
Die Funktionen zum Starten von asynchronen Leseoperationen auf Handles
wurden in start\_read() ausgelagert und das Übersetzen von Fehlercodes in menschenlesbare
Texte in format\_error(). Bei Fehlern beim Lesen startet sich der Job
...
\\
\\
\bitlabel{5}{Notification-Event} &
\bitlabel{5}{this->tun} &
\bitlabel{5}{Restliche Geräte} \\
\begin{leftwordgroup}{event\_array}
\bitbox{5}{this->event}
\bitbox{5}{Event für this->tun}
\bitbox{5}{\dots} 
\end{leftwordgroup} \\
\begin{leftwordgroup}{bundle\_array}
\bitbox{5}{dummy-Struct}
\bitbox{5}{Struct für this->tun}
\bitbox{5}{\dots}
\end{leftwordgroup} \\
\end{bytefield}
\label{fig:arrays}
...
In handle\_plain werden zwei verschiedene Arrays benutzt, deren Strukturen in~\autoref{fig:arrays}
dargestellt sind.
...
vom Typ HANDLE, ein Objekt vom Typ \texttt{*OVERLAPPED} und ein Objekt vom Typ chunk\_t.
Das Handle gehört zu einem TUN-Gerät, das \texttt{OVERLAPPED}-Objekt wird direkt für asynchrones
...
Das \texttt{HANDLE} wird dabei von der Funktoon \texttt{init\_tun()} gesetzt,
welche beim Instanzieren der Klasse im Konstruktur aufgerufen wird.
